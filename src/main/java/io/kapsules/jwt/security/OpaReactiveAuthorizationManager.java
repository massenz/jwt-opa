...
@Component
@Slf4j
public class OpaReactiveAuthorizationManager implements
  ...
  private Mono<String> resolveToken(ServerHttpRequest request) {
    ...
    if (StringUtils.hasText(bearerToken) && bearerToken.startsWith(PREFIX)) {
      return Mono.just(bearerToken.substring(PREFIX.length() + 1));
    }
   ...
   *
   * @param exchange the Exchange, which carries the request, inside whose Authorization header the
   *                 API token is carried ({@literal Bearer} token)
   * @return a non-empty Mono if the token is validated, with the API token inside
   */
  private Mono<String> validate(ServerWebExchange exchange) {
  ...
  public Mono<AuthorizationDecision> check(Mono<Authentication> authentication,
    ...
    return authentication
        ...
        .flatMap(b -> b ?
            ...
            validate(context.getExchange())
                .map(token -> {
                  log.warn("OPA Authorization not implemented, allowing everyone");
                  // TODO: here is where we need to inject OPA authorization
